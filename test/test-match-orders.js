...
describe("Match Orders Tests", () => {
  ...
  it("should recognize ERC2981Royalties interface through the Royalties registry - without failure when there are no royalties", async () => {
    const { universeMarketplace, mockNFT4 } = await loadFixture(
      deployedContracts
    );
    ...
    await mockNFT4
      .connect(accounts[0])
      .mint("https://universe.xyz", 0, ethers.constants.AddressZero);
    await mockNFT4.connect(accounts[0]).approve(universeMarketplace.address, 1);
    ...
    expect(balanceAfter.sub(balanceBefore)).to.equal(75);
    expect(
      balanceRoyaltyReceiverAfter.sub(balanceRoyaltyReceiverBefore)
    ).to.equal(0);
